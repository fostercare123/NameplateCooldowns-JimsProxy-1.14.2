...
local _G, pairs, UIParent, string_gsub, string_find, bit_band, GetTime, math_ceil, table_insert, table_sort, C_Timer_After, string_format, C_Timer_NewTimer, math_max, C_NamePlate_GetNamePlateForUnit, UnitGUID =
	  _G, pairs, UIParent, string.gsub,	string.find, bit.band, GetTime, math.ceil, table.insert, table.sort, C_Timer and C_Timer.After, string.format, C_Timer and C_Timer.NewTimer, math.max, C_NamePlate and C_NamePlate.GetNamePlateForUnit, UnitGUID;
		...
		for class, cds in pairs(addonTable.CDs) do
			for spellId, cd in pairs(cds) do
				if (SpellNameByID[spellId] ~= nil) then
					...
					addonTable.SpellIDByName[SpellNameByID[spellId]] = spellId;
...
do
	...
	local function refreshCDs()
		...
		for _, unitGUID in pairs(NameplatesVisible) do
			...
			SpellsPerPlayerGUID[unitGUID][18834] = { ["spellID"] = 18834, ["expires"] = cTime + 120, ["texture"] = SpellTextureByID[18834] }; -- // 2m test
	...
	function addonTable.EnableTestMode()
		...
		db.SpellCDs[18834] = GetDefaultDBEntryForSpell();
		db.SpellCDs[18834].enabled = true;
		db.SpellCDs[18834].glow = GLOW_TIME_INFINITE;
...
do
	...
	EventFrame.COMBAT_LOG_EVENT_UNFILTERED = function()
		...
		if (not spellID or spellID == 0) and spellName then
			spellID = addonTable.SpellIDByName[spellName];
		end
		if not spellID then return end

		local isHostile = (srcFlags and bit_band(srcFlags, COMBATLOG_OBJECT_REACTION_HOSTILE or 0x00000040) ~= 0)
		if (isHostile or (db.ShowCDOnAllies == true and srcGUID ~= LocalPlayerGUID)) then
